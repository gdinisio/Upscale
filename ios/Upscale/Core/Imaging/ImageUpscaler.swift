...
private struct CoreMLStrategy {
    ...
    let primaryUpscaleModel: CoreMLModelDescriptor
    let secondaryUpscaleModel: CoreMLModelDescriptor?
}
...
struct ImageUpscaler {
    ...
    private func upscaleWithBundledCoreMLModel(
        ...
        while remainingScale > 1.01, passCount < maxPasses {
            let primaryPass = try runCoreMLPassWithAugmentations(
                on: currentImage,
                model: strategy.primaryUpscaleModel,
                augmentations: augmentations
            )
            inferencePassCount += augmentations.count

            let combinedPass: CoreMLPassResult
            if let secondaryModel = strategy.secondaryUpscaleModel {
                let secondaryPass = try runCoreMLPassWithAugmentations(
                    on: currentImage,
                    model: secondaryModel,
                    augmentations: augmentations
                )
                inferencePassCount += augmentations.count
                combinedPass = try mergeUpscalePassResults(primary: primaryPass, secondary: secondaryPass)
            } else {
                combinedPass = primaryPass
            }

            let effectiveScale = min(combinedPass.scaleX, combinedPass.scaleY)
            ...
            if combinedPass.image.width != normalizedWidth || combinedPass.image.height != normalizedHeight {
                let correctionScaleX = Double(normalizedWidth) / Double(max(combinedPass.image.width, 1))
                let correctionScaleY = Double(normalizedHeight) / Double(max(combinedPass.image.height, 1))
                currentImage = try lanczosResample(
                    combinedPass.image,
                ...
                currentImage = combinedPass.image
            }

            remainingScale /= effectiveScale
            passCount += 1

            let hint = max(strategy.primaryUpscaleModel.nominalScale, 1.0)
        ...
        stageLabels.append(strategy.primaryUpscaleModel.name)
        if let secondaryModel = strategy.secondaryUpscaleModel {
            stageLabels.append("ensemble \(secondaryModel.name)")
        }
    ...
    private func selectCoreMLStrategy(
        ...
        guard !upscalers.isEmpty else { return nil }

        switch qualityMode {
        case .fast:
            guard let model = preferredFastUpscaleModel(from: upscalers) else { return nil }
            return CoreMLStrategy(restorationModel: nil, primaryUpscaleModel: model, secondaryUpscaleModel: nil)
        case .balanced:
            guard let model = preferredHighQualityUpscaleModel(from: upscalers) else { return nil }
            let restorers = models.filter { $0.nominalScale <= 1.01 && $0.name != model.name }
            let restorationModel = preferredRestorationModel(from: restorers)
            return CoreMLStrategy(restorationModel: restorationModel, primaryUpscaleModel: model, secondaryUpscaleModel: nil)
        case .ultra:
            guard let primary = preferredHighQualityUpscaleModel(from: upscalers) else { return nil }
            let secondary = preferredSecondaryUpscaleModel(primary: primary, from: upscalers)
            let restorers = models.filter { $0.nominalScale <= 1.01 && $0.name != primary.name }
            let restorationModel = preferredRestorationModel(from: restorers)
            return CoreMLStrategy(restorationModel: restorationModel, primaryUpscaleModel: primary, secondaryUpscaleModel: secondary)
        }
    }

    private func preferredFastUpscaleModel(from candidates: [CoreMLModelDescriptor]) -> CoreMLModelDescriptor? {
        let preferredNames = [
            "RealESRGAN",
            "RealESRGAN_x4",
            "RealESRGANx4",
        ...
        return firstMatchingModel(from: candidates, preferredNames: preferredNames)
            ?? candidates.max(by: { $0.nominalScale < $1.nominalScale })
    }

    private func preferredHighQualityUpscaleModel(from candidates: [CoreMLModelDescriptor]) -> CoreMLModelDescriptor? {
        let preferredNames = [
            "MMRealSRGAN",
            "RealESRGAN",
            "RealESRGAN_x4",
    ...
    private func preferredSecondaryUpscaleModel(
        primary: CoreMLModelDescriptor,
        from candidates: [CoreMLModelDescriptor]
    ) -> CoreMLModelDescriptor? {
        let filtered = candidates.filter { $0.name != primary.name }
        ...
        return firstMatchingModel(from: filtered, preferredNames: preferredNames) ?? filtered.first
    }

    private func firstMatchingModel(
        from candidates: [CoreMLModelDescriptor],
        preferredNames: [String]
    ) -> CoreMLModelDescriptor? {
        for preferredName in preferredNames {
            if let exact = candidates.first(where: { $0.name.caseInsensitiveCompare(preferredName) == .orderedSame }) {
                return exact
            }
            if let contains = candidates.first(where: { $0.name.lowercased().contains(preferredName.lowercased()) }) {
                return contains
            }
        }
    ...
    private func mergeUpscalePassResults(
        primary: CoreMLPassResult,
        secondary: CoreMLPassResult
    ) throws -> CoreMLPassResult {
        let targetWidth = max(primary.image.width, secondary.image.width)
        let targetHeight = max(primary.image.height, secondary.image.height)

        let primaryAligned: CGImage
        if primary.image.width == targetWidth, primary.image.height == targetHeight {
            primaryAligned = primary.image
        } else {
            primaryAligned = try lanczosResample(
                primary.image,
                scaleX: Double(targetWidth) / Double(max(primary.image.width, 1)),
                scaleY: Double(targetHeight) / Double(max(primary.image.height, 1))
            )
        }

        let secondaryAligned: CGImage
        if secondary.image.width == targetWidth, secondary.image.height == targetHeight {
            secondaryAligned = secondary.image
        } else {
            secondaryAligned = try lanczosResample(
                secondary.image,
                scaleX: Double(targetWidth) / Double(max(secondary.image.width, 1)),
                scaleY: Double(targetHeight) / Double(max(secondary.image.height, 1))
            )
        }

        let blended = try detailAwareBlend(primary: primaryAligned, secondary: secondaryAligned)
        let mergedScaleX = (primary.scaleX + secondary.scaleX) / 2.0
        let mergedScaleY = (primary.scaleY + secondary.scaleY) / 2.0

        return CoreMLPassResult(image: blended, scaleX: mergedScaleX, scaleY: mergedScaleY)
    }

    private func detailAwareBlend(primary: CGImage, secondary: CGImage) throws -> CGImage {
        let primaryImage = CIImage(cgImage: primary)
        let secondaryImage = CIImage(cgImage: secondary)
        let extent = CGRect(x: 0, y: 0, width: primary.width, height: primary.height)

        let edgeMask = primaryImage
        ...
        blend.inputImage = primaryImage
        blend.backgroundImage = secondaryImage
        blend.maskImage = edgeMask

        let mixed = blend.outputImage ?? primaryImage

        let stabilize = CIFilter.dissolveTransition()
        stabilize.inputImage = mixed
        stabilize.targetImage = secondaryImage
        stabilize.time = 0.18

        let outputImage = (stabilize.outputImage ?? mixed).cropped(to: extent)
    ...
    private func scaleHint(from modelName: String) -> Double? {
        ...
        if normalized.contains("mmrealsrgan") {
            return 4.0
        }
